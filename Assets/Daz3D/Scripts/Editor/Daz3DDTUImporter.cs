...
*/
        ...
        public static void GeneratePrefabFromFBX(string fbxPath, DazFigurePlatform platform, DTU dtu)
        {
            ...
            */
            ...
            // Send To PlayGrounds (2022-Mar-14, DB)
            GameObject mainCamera = Camera.main.gameObject;
            if (mainCamera == null || mainCamera.GetComponent<Cinemachine.CinemachineBrain>() == null)
            {
                if (mainCamera != null) mainCamera.SetActive(false);
                var mainCameraPrefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Prefabs/MainCamera.prefab");
                mainCamera = Instantiate<GameObject>(mainCameraPrefab);
                mainCamera.name = "MainCamera";
            }
            GameObject followCamera = GameObject.Find("PlayerFollowCamera");
            if (followCamera == null || followCamera.GetComponent<Cinemachine.CinemachineVirtualCamera>() == null)
            {
                if (followCamera != null) DestroyImmediate(followCamera);
                var followCameraPrefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Prefabs/PlayerFollowCamera.prefab");
                followCamera = Instantiate<GameObject>(followCameraPrefab);
                followCamera.name = "PlayerFollowCamera";
            }
            GameObject avatarTemplate = GameObject.Find("AvatarTemplate");
            if (avatarTemplate == null)
            {
                var avatarTemplatePrefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Prefabs/AvatarTemplate.prefab");
                avatarTemplate = Instantiate<GameObject>(avatarTemplatePrefab);
                avatarTemplate.name = "AvatarTemplate";
                // Link Player Camera
                var avatarCameraRoot = avatarTemplate.transform.Find("AvatarCameraRoot");
                followCamera.GetComponent<Cinemachine.CinemachineVirtualCamera>().Follow = avatarCameraRoot;
            }
            if (avatarTemplate)
            {
                resultingInstance.transform.SetParent(avatarTemplate.transform, false);
                resultingInstance.transform.localPosition = new Vector3(0, 0.025f, 0);
                avatarTemplate.GetComponent<Animator>().avatar = resultingInstance.GetComponent<Animator>().avatar;
            }
